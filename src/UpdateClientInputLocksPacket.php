...
class UpdateClientInputLocksPacket extends DataPacket implements ClientboundPacket{
	 ...
	 */
	public static function create(int $locks, Vector3 $position) : self{
		$result = new self;
		$result->locks = $locks;
	...
	public function getLockFlags() : int{ return $this->locks; }

	public function getPosition() : Vector3{ return $this->position; }

	protected function decodePayload(PacketSerializer $in) : void{
		$this->locks = $in->getUnsignedVarInt();
		$this->position = $in->getVector3();
	}

	protected function encodePayload(PacketSerializer $out) : void{
		$out->putUnsignedVarInt($this->locks);
